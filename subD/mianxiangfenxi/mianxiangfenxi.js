      ...
      success: (res) => {
        ...
        wx.getFileSystemManager().readFile({
          ...
          success: fileRes => {
            ...
            wx.setStorageSync('faceImagePath', res.tempImagePath)
            wx.setStorageSync('faceBase64', fileRes.data)
            console.log('[mianxiangfenxi] faceImagePath:', res.tempImagePath)
            console.log('[mianxiangfenxi] faceBase64 length:', fileRes.data.length)
            wx.navigateTo({ url: `/subD/mxloading/mxloading?type=${this.data.type}` })
          },
          fail: () => {
            wx.hideLoading()
      ...
      success: (res) => {
        ...
        wx.getFileSystemManager().readFile({
          ...
          success: fileRes => {
            wx.hideLoading()
            wx.removeStorageSync('rawApiResponse')
            wx.setStorageSync('faceImagePath', tempImagePath)
            wx.setStorageSync('faceBase64', fileRes.data)
            console.log('[mianxiangfenxi] faceImagePath:', tempImagePath)
            console.log('[mianxiangfenxi] faceBase64 length:', fileRes.data.length)
            wx.navigateTo({ url: `/subD/mxloading/mxloading?type=${this.data.type}` })
          },
          fail: () => {
