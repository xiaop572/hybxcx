...
Page({
  ...
  // 计算最终价格
  calculateFinalPrice() {
    let totalPrice = this.data.item.price // 直接使用产品单价，不计算数量
    let discountAmount = 0
    
    if (this.data.hasAvailableCoupons && this.data.availableCoupons.length > 0) {
      // 使用第一个可用优惠券的折扣金额
      discountAmount = this.data.availableCoupons[0].DiscountValue || 0
    }
    
    let finalPrice = Math.max(0, totalPrice - discountAmount) // 确保价格不为负数
  ...
  payment() {
    ...
    let requestData = {
      ...
      "TotalFee": that.data.hasAvailableCoupons ? that.data.finalPrice : that.data.item.price,
      "openId": wx.getStorageSync('openid'),
