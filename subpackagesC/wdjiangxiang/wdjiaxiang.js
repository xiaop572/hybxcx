...
Page({
  ...
  gettjprodwzn(){
    req({
      url: util.baseUrl + "/newapi/api/tj/gettjprodwzn",
      method: "post",
      data: {
        curpage: 1,
        limint: 999999
  ...
  selectItem(e) {
    ...
    if (this.data.sex === 'nan') {
      ...
      // 女性最多选择2项
      let selectedCount = this.data.xmList.filter(item => item.select).length;
      if (!this.data.xmList[index].select && selectedCount >= 2) {
        wx.showModal({
          title: '提示',
          content: '最多只能选择2个项目',
          showCancel: false
      ...
      this.setData({
        [`xmList[${index}].select`]: !this.data.xmList[index].select,
        sum: 0,
        yuanjia: 0
      });
    }
  ...
  payment() {
    ...
    if (this.data.sex === 'nan' && selectedItems.length === 0) {
      wx.showModal({
        title: '提示',
        content: '请选择一项',
        showCancel: false
    ...
    if (this.data.sex === 'nv' && selectedItems.length < 2) {
      wx.showModal({
        title: '提示',
        content: '请选择两项',
        showCancel: false
