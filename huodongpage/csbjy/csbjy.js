const {
  req
} = require('../../utils/request')
const util = require('../../utils/util');

// huodongpage/jskq/jskq.js
Page({

  /**
   * 页面的初始数据
   */
  data: {
    isAuthorized: true,
    cardImage: '',
    type: '',
    showModal: false,
    usage: '',
    phoneNumber: '',
    shareCode: '',
    showClaimModal: false,
    claimName: '',
    claimPhone: '',
    money: 0,
    proid: 0,
    isClaiming: false
  },

  /**
   * 生命周期函数--监听页面加载
   */
  onLoad(options) {
    console.log(options)
    const type = options.type;
    let cardImage = '';
    
    if (type == 1) {
      // 院长
      cardImage = 'https://wx.pmc-wz.com/materials/jskqyz.png';
    } else if (type == 2) {
      // 中高层
      cardImage = 'https://wx.pmc-wz.com/materials/jskqgc.png';
    }

    this.setData({
      cardImage,
      type,
      shareCode: options.shareCode || '',
      money: Number(options.money || 0),
      proid: Number(options.proid || 0)
    });
  },

  handleClaim() {
    this.setData({
      showClaimModal: true,
      claimName: '',
      claimPhone: ''
    });
  },

  hideClaimModal() {
    this.setData({ showClaimModal: false });
  },

  onNameInput(e) {
    this.setData({ claimName: e.detail.value });
  },

  onClaimPhoneInput(e) {
    this.setData({ claimPhone: e.detail.value });
  },

  getPhoneNumber(e) {
    if (e.detail.iv && e.detail.encryptedData) {
      req({
        url: util.baseUrl + '/newapi/api/WechatUser/getwxmobile2',
        method: 'POST',
        data: {
          openid: wx.getStorageSync('openid'),
          iv: e.detail.iv,
          encryptedData: e.detail.encryptedData,
          session_key: wx.getStorageSync('sessionKey')
        },
        success: res => {
          if (res.data.status && res.data.data) {
            this.setData({ claimPhone: res.data.data });
            wx.showToast({ title: '获取成功', icon: 'success' });
          } else {
            wx.showToast({ title: '获取手机号失败', icon: 'none' });
          }
        },
        fail: () => {
          wx.showToast({ title: '获取手机号失败', icon: 'none' });
        }
      });
    } else {
      wx.showToast({ title: '授权失败，请重试', icon: 'none' });
    }
  },

  confirmClaim() {
    const { claimName, claimPhone, money, proid } = this.data;
    if (this.data.isClaiming) {
      return;
    }
    if (!claimName) {
      wx.showToast({ title: '请输入姓名', icon: 'none' });
      return;
    }
    if (!claimPhone || claimPhone.length !== 11) {
      wx.showToast({ title: '请输入正确的手机号', icon: 'none' });
      return;
    }
    if (!wx.getStorageSync('token') || !wx.getStorageSync('openid')) {
      wx.showModal({
        title: '提示',
        content: '请登录后使用',
        complete: (res) => {
          if (res.confirm) {
            wx.navigateTo({
              url: '/pages/login/login',
            });
          }
        }
      });
      return;
    }
    this.setData({ showClaimModal: false });
    this.setData({ isClaiming: true });
    wx.showLoading({ title: '领取中...' });
    req({
      url: util.baseUrl + '/newapi/api/topic/givenyykouqian',
      method: 'POST',
      data: {
        openid: wx.getStorageSync('openid'),
        money: Number(money || 0),
        proid: Number(proid || 0),
        xinmin: claimName,
        mobile: claimPhone
      },
      success: (res) => {
        if (res.data.status) {
          wx.showModal({
            title: '提示',
            content: '领取成功',
            showCancel:false,
            complete: (res) => {
              wx.navigateTo({
                url: '/subpackages/mycard/mycard',
              })
            }
          })
        } else {
          wx.showModal({
            title: '提示',
            content: res.data.data,
            showCancel:false
          })
        }
      },
      fail: (err) => {
        console.error('领取失败', err);
        wx.showToast({ title: '网络错误', icon: 'none' });
      },
      complete: () => {
        wx.hideLoading();
        this.setData({ isClaiming: false });
      }
    });
  },
  rmykq(){
    wx.navigateTo({
      url: '/subpackages/mycard/mycard',
    })
  },
  /**
   * 生命周期函数--监听页面初次渲染完成
   */
  onReady() {

  },

  /**
   * 生命周期函数--监听页面显示
   */
  onShow() {
    wx.hideLoading();
    if (this.data.isClaiming) {
      this.setData({ isClaiming: false });
    }
  },

  /**
   * 生命周期函数--监听页面隐藏
   */
  onHide() {
    wx.hideLoading();
  },

  /**
   * 生命周期函数--监听页面卸载
   */
  onUnload() {

  },

  /**
   * 页面相关事件处理函数--监听用户下拉动作
   */
  onPullDownRefresh() {

  },

  /**
   * 页面上拉触底事件的处理函数
   */
  onReachBottom() {

  },

  /**
   * 用户点击右上角分享
   */
  onShareAppMessage() {
    return {
      title: '给牙齿做一次SPA！点击领取专属洁牙礼遇',
      imageUrl: "https://wx.pmc-wz.com/materials/csbjyftx.jpg",
      path: `/huodongpage/csbjy/csbjy`
    }
  },

  // 页面交互逻辑（防报错占位）
  showShareModal() {
    this.setData({ showModal: true });
  },

  hideShareModal() {
    this.setData({ showModal: false });
  },

  preventBubble() {},

  onUsageInput(e) {
    this.setData({ usage: e.detail.value });
  },

  onPhoneInput(e) {
    this.setData({ phoneNumber: e.detail.value });
  },

  handleShareSuccess() {
    // 实际分享逻辑由 open-type="share" 触发
    this.hideShareModal();
  },

  handleShareError() {
    wx.showToast({
      title: '请填写完整信息',
      icon: 'none'
    });
  }
})
