...
Page({
  ...
  confirmClaim() {
    const { claimName, claimPhone, money, proid } = this.data;
    if (!claimName) {
    ...
    req({
      url: util.baseUrl + '/newapi/api/huodong/givejkmkaquan',
      method: 'POST',
      data: {
        openid: wx.getStorageSync('openid'),
        money: Number(money || 0),
        proid: Number(proid || 0),
        xinmin: claimName,
        mobile: claimPhone
      },
      success: (res) => {
        ...
        if (res.data.status) {
          ...
          wx.showToast({ title: res.data.msg || res.data.data || '领取失败', icon: 'none' });
        }
      },
      fail: (err) => {
        wx.hideLoading();
  ...
  onShareAppMessage() {
    return {
      title: '限时解锁健康美新体验，点击领取专属权益',
      imageUrl: "https://wx.pmc-wz.com/materials/jkmlbfxt.jpg",
      path: `/huodongpage/jkmzxlb/jkmzxlb`
    }
  },
