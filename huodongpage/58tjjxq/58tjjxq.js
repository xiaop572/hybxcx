...
Page({
  ...
  getRandomCard() {
    ...
    req({
      url: baseUrl + '/newapi/api/topic/givenbbankm22',
      method: 'POST',
      data: {
        openid: openid,
        money:0,
  ...
  getPhoneNumber(e) {
    ...
    req({
      url: baseUrl + '/newapi/api/WechatUser/getwxmobile2',
      method: 'POST',
      data: {
  ...
  claimCard() {
    const openid = wx.getStorageSync('openid')
    const { claimName, claimPhone } = this.data
    if (!claimName) {
      wx.showToast({
        title: '请输入姓名',
        icon: 'none'
    ...
    if (!claimPhone || String(claimPhone).length !== 11) {
      wx.showToast({
        title: '请先获取手机号',
        icon: 'none'
      })
    ...
    req({
      url: baseUrl + '/newapi/api/topic/give58jiaxiang',
      method: 'POST',
      data: {
        openid: openid,
        money:0,
